...
class Priority_Queue:
    ...
    def split_key(self, key):
        ...
        -------------------------------------------------------
        """
        target1 = Priority_Queue()  # Priority queue for values with priority > key
        target2 = Priority_Queue()  # Priority queue for values with priority <= key

        while not self.is_empty():
            value = self.remove()  # Remove a value from the source queue
            if value <= key:
                # Insert into target2 if priority is less than or equal to key
                target2.insert(value)
            else:
                # Insert into target1 if priority is greater than key
